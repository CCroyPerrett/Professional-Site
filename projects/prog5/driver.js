...
var point = false; 
var orig_pointloc = [(-100 - 250)/250, (100 - 250)/250, (100 - 250)/250];
...
function addEvntLsnr18(){
  var xcam = document.getElementById("xcam");
  let update = () => xcam.value;

  xcam.addEventListener('input', update);
  update();
}
function addEvntLsnr19(){
  var ycam = document.getElementById("ycam");
  let update = () => ycam.value;

  ycam.addEventListener('input', update);
  update();
}
function addEvntLsnr20(){
  var zcam = document.getElementById("zcam");
  let update = () => zcam.value;

  zcam.addEventListener('input', update);
  update();
}
function addEvntLsnr21(){
  var zzoom = document.getElementById("zoom");
  let update = () => zzoom.value;

  zzoom.addEventListener('input', update);
  update();
}
function addEvntLsnr22(){
  var bred = document.getElementById("bred");
  let update = () => bred.value;

  bred.addEventListener('input', update);
  update();
}

function addEvntLsnr23(){
  var bblue = document.getElementById("bblue");
  let update = () => bblue.value;

  bblue.addEventListener('input', update);
  update();
}

function addEvntLsnr24(){
  var bgreen = document.getElementById("bgreen");
  let update = () => bgreen.value;

  bgreen.addEventListener('input', update);
  update();
}

function addEvntLsnr25(){
  var opacity = document.getElementById("opacity");
  let update = () => opacity.value;
...
function toggle_ortho(){
  if(orthogonal == false){
    orthogonal = true;
    perspective = false;
    //console.log("perspective is now: " + perspective);
...
function FindNormals(skeleton, surf){
  ...
  for(var i = 0; i < surf.length; i++){
    ...
    //}
    normals.push(normal);
    //console.log("normals are: " + normal.x);
  }
...
function FillColor(contxt, normal, skeleton, surf, clr, opacity){
  ...
  for(var i = 0; i < ordered_surf.length; i++){
    ...
    if(flat && camCosAngle > 0){
      ...
      var sidecolor = CalculateLight(sidecoords, [rgbx, rgby, rgbz], pointloc, pointcol, normal[ordered_surf[i][0]], (ordered_surf[i][0] % 2 == 1), true)
      //fixes problem where every second triangle is reversed
      /*if(ordered_surf[i][0] % 2 == 1){
        lightCosAngle *= -1;
      }
      ...
      sidecolor = "#" + redstr + greenstr + bluestr;*/
      contxt.fillStyle = "rgba("+sidecolor[0]+ "," + sidecolor[1]+","+sidecolor[2]+","+opacity+")";
      //console.log("opacity is: " + opacity);
      //contxt.fillStyle = rgba(,,, opacity)
      //console.log("sidecoloris: " + sidecolor);
      contxt.moveTo((p1.x+1) * 250, (p1.y+1) * 250);
      contxt.lineTo((p2.x+1) * 250, (p2.y+1) * 250);
      contxt.lineTo((p3.x+1) * 250, (p3.y+1) * 250);
      contxt.fill();
    ...
    for(var i = 0; i < ordered_surf.length; i++){
      ...
      if(camCosAngle > 0){
        ...
        for(var j = 0; j < 3; j++){
          ...
          colors[j] = CalculateLight(thiscor, [rgbx, rgby, rgbz],  pointloc, pointcol, normal[ordered_surf[i][0]], (ordered_surf[i][0] % 2 == 1), false);
  ...
  if(pointloc[2] > (avarage_z/zcount) && showpoint){
    ShowPoint(world_cont);
  }
}
...
function Finish(){
  ...
  if(allow_new_points == false){
    ...
    world_opacity_matrix.push(1);
    //console.log('world opacity matrix is: ' + world_opacity_matrix);
...
function ShowPoint(contxt){
 // console.log("show point, point is: " + ((pointloc[0] + 1) * 250) + ", " + ((pointloc[1] + 1) * 250));
  //contxt.beginPath();
  //world_cont.strokeStyle = "black";
  //world_cont.moveTo((pointloc[0] + 1) * 250,(pointloc[1].y  + 1) * 250);
  //world_cont.lineTo((pointloc[0] + 1) * 250 + 10,(pointloc[1].y  + 1) * 250 + 10);
  //world_cont.arc((pointloc[0] + 1) * 250, (pointloc[1].y  + 1) * 250, 20, 0, 2 * Math.PI, false);

  
  var xloc = Math.floor((pointloc[0] + 1) * 250); var yloc = Math.floor((pointloc[1]  + 1) * 250);
  //console.log("show point, point is: " + xloc + ", " + yloc);
  contxt.fillStyle = 'orange';
  contxt.fillRect(xloc-1, yloc-1, 10, 10);
  //contxt.fill();
  //world_cont.lineWidth = 5;
  //world_cont.strokeStyle = '#003300';
  //world_cont.stroke();
}
...
function DrawWorld(){
  ...
  for(var i = 0; i < world_skel.length ; i++){
    ...
    //does normal rotations for objects 
    ...
    //rotates camera around objects
    var camera_rota_skel = RotateSkel(trans_skel, camrota);

    //if perspective view , make farther points closer to orogin
    if(perspective){
      //console.log("doing perspective scaling")
      camera_rota_skel = PerspectSkel(camera_rota_skel);
    }

    //maks objects smaller or bigger according to zoom
    var zoomed_skel = ZoomSkel(camera_rota_skel);

    var normal = FindNormals(zoomed_skel,  world_surfaces[i]);

    normals.push(normal);
    transformed_world.push(zoomed_skel);
  }
  ...
  var trans_copy = [];
  for(var i = 0; i < transformed_world.length ; i++){
    var trans_index = [i, AvgZ(transformed_world[i])];
    trans_copy.push(trans_index);

    //swap objects untill they are correct order
    for(var j = 1; j < trans_copy.length; j++){
      //if frontz is less than backz, swap 
      if(trans_copy[trans_copy.length - j][1] < trans_copy[trans_copy.length - j - 1][1]){
        var neworder = trans_copy[trans_copy.length - j][0];
        var oldorder = trans_copy[trans_copy.length - j - 1][0];
        var newval = trans_copy[trans_copy.length - j][1];
  ...
  for(var i = 0; i < trans_copy.length ; i++){
    ...
    if(lines || draw_wire == true){
      DrawWireframe(transformed_world[sel_obj], world_cont);
    }
    if(draw_wire == false){
      FillColor(world_cont, normals[sel_obj], transformed_world[sel_obj], world_surfaces[sel_obj], world_colors[sel_obj], world_opacity_matrix[sel_obj]);
    }
  }
...
function updateInputs(){
  ...
  if(sel_obj < world_scale.length){
    ...
    document.getElementById("opacity").value = world_opacity_matrix[sel_obj];
  }
...
function ChangeOpacity(){
  var opacity = document.getElementById("opacity").value;
  var sel_obj = document.getElementById("transselect").value;
  world_opacity_matrix[sel_obj] = opacity;
  DrawWorld();
}
...
function PerspectSkel(skeleton){
  var scaled_skel = [];
  
  for(var i = 0; i < skeleton.length; i++){
    var segment = [];
    for(var j = 0; j < skeleton[i].length; j++){
      var perpscale = 10/(9+skeleton[i][j].z);
      var newpoint = {
        x: skeleton[i][j].x * 2/(2-skeleton[i][j].z),
        y: skeleton[i][j].y * 2/(2-skeleton[i][j].z),
        z: skeleton[i][j].z * 2/(2-skeleton[i][j].z)
      }
      segment.push(newpoint);
    }
    scaled_skel.push(segment);
...
function ChangeCam(){
    var xcam = document.getElementById("xcam").value;
    var ycam= document.getElementById("ycam").value;
    var zcam = document.getElementById("zcam").value;
    var newcam = [xcam, ycam, zcam];
    camrota = newcam;
    UpdateLight()
    DrawWorld();
}
...
function ChangePoint(){
    ...
    orig_pointloc= [xpoint, ypoint, zpoint];
...
function UpdateLight(){
  //onsole.log("light updates");
  //holds origional light direction for directional
  ld = [1,1,1]

  //does x rotation
  var xld = [0,0,0];
  xld[0] = ld[0];
  xld[1] = ld[1] * Math.cos(camrota[0] * Math.PI / 180) - ld[2] * Math.sin(camrota[0] * Math.PI / 180);
  xld[2] = ld[1] * Math.sin(camrota[0] * Math.PI / 180) + ld[2] * Math.cos(camrota[0] * Math.PI / 180);

  //does y rotation
  var yld = [0,0,0];
  yld[0] = xld[0] * Math.cos(camrota[1]* Math.PI / 180) + xld[2] * Math.sin(camrota[1]* Math.PI / 180);
  yld[1] = xld[1];
  yld[2] = (-1) *  xld[0] * Math.sin(camrota[1]* Math.PI / 180) + xld[2] * Math.cos(camrota[1]* Math.PI / 180);

  //does z rotation
  var zld = [0,0,0];
  zld[0] = yld[0] * Math.cos(camrota[2]* Math.PI / 180) - yld[1] * Math.sin(camrota[2]* Math.PI / 180);
  zld[1] = yld[0] * Math.sin(camrota[2]* Math.PI / 180) + yld[1] * Math.cos(camrota[2]* Math.PI / 180);
  zld[2] = yld[2];

  //updates directional light's direction
  lightdir = Normalize(zld);
  lightdir = [lightdir[0]* 3, lightdir[1]* 3, lightdir[2]* 3];

  // holds origional light direction for point
  
  opl = [orig_pointloc[0] * (1.2+zoom/100), orig_pointloc[1] * (1.2+zoom/100), orig_pointloc[2] * (1.2+zoom/100)];
  //does x rotation
  var xpl = [0,0,0];
  xpl[0] = opl[0];
  xpl[1] = (-1)*opl[1] * Math.cos(camrota[0] * Math.PI / 180) - opl[2] * Math.sin(camrota[0] * Math.PI / 180);
  xpl[2] = (-1)*opl[1] * Math.sin(camrota[0] * Math.PI / 180) + opl[2] * Math.cos(camrota[0] * Math.PI / 180);

  //does y rotation
  var ypl = [0,0,0];
  ypl[0] = xpl[0] * Math.cos(camrota[1]* Math.PI / 180) + xpl[2] * Math.sin(camrota[1]* Math.PI / 180);
  ypl[1] = xpl[1];
  ypl[2] = (-1) *  xpl[0] * Math.sin(camrota[1]* Math.PI / 180) + xpl[2] * Math.cos(camrota[1]* Math.PI / 180);

  //does z rotation
  var zpl = [0,0,0];
  zpl[0] = ypl[0] * Math.cos(camrota[2]* Math.PI / 180) - ypl[1] * Math.sin(camrota[2]* Math.PI / 180);
  zpl[1] = ypl[0] * Math.sin(camrota[2]* Math.PI / 180) + ypl[1] * Math.cos(camrota[2]* Math.PI / 180);
  zpl[2] = ypl[2];

  //updates directional light's direction
  //zpl = [zpl[0] * (1.2+zoom/100), zpl[1] * (1.2+zoom/100), zpl[2] * (1.2+zoom/100)]
  //pointloc = Normalize(zpl);
  pointloc = zpl;
  //lightdir = [lightdir[0]* 3, lightdir[1]* 3, lightdir[2]* 3];
  //pointloc = orig_pointloc;
}

function Normalize(vect){
  var magnitude = Math.sqrt(vect[0]**2 + vect[1]**2 + vect[2]**2);
  vect = [vect[0]/magnitude, vect[1]/magnitude, vect[2]/magnitude]
  return vect;
}

function AvgZ(skeleton){
  average_z = 0;
  zcount = 0;

  for(var i = 0; i < skeleton.length; i++){
    for(var j = 0; j < skeleton[i].length; j++){
      average_z += skeleton[i][j].z;
      zcount;
    }
  }

  return (average_z/zcount);
}
...
function ZoomSkel(skeleton){
  ...
  for(var i = 0; i < skeleton.length; i++){
    ...
    for(var j = 0; j < skeleton[i].length; j++){
      var newpoint = {
        x: skeleton[i][j].x * (1.2+zoom/100),
        y: skeleton[i][j].y * (1.2+zoom/100),
        z: skeleton[i][j].z * (1.2+zoom/100),
      }
      segment.push(newpoint);
    }
    zoomed_skel.push(segment);
  }
  return zoomed_skel;
}
...
function CalculateLight(thiscor, rgb, lightcoor, lightcol, normal, islower, doarr){
  var ld = lightdir;
  var color;
  var normalMag = Math.sqrt(normal.x ** 2 + normal.y ** 2 + normal.z ** 2);
  //var lightMag = Math.sqrt(1**2 + 1**2 + 1**2);
  var lightMag = Math.sqrt(ld[0]**2 + ld[1]**2 + ld[2]**2);
  ///var lightDotProduct = 1*thiscor.x + 1*thiscor.y + 1*thiscor.z;
  //var lightDotProduct = lightdir[0]*normal.x + lightdir[1]*normal.y + lightdir[2]*normal.z;
  var lightDotProduct = ld[0]*normal.x + ld[1]*normal.y + ld[2]*normal.z;
  var lightCosAngle = lightDotProduct/(normalMag * lightMag);

  //console.log("Light Direction is: [" + lightdir[0] + ", "  + lightdir[1] + ", "  + lightdir[2] + "]");
  //console.log("Cam rotation is: [" + lightdir[0] + ", "  + lightdir[1] + ", "  + lightdir[2] + "]");

  //fixes problem where every other triangle faces backwards
  if(islower){
    lightCosAngle *= -1;
  }
  ...
  var lightPointDotProduct = lightcoor[0]*normal.x + lightcoor[1]*normal.y + lightcoor[2]*normal.z;
  var lightPointCosAngle = lightPointDotProduct/(normalMag * lightPointMag);
  //fixes problem where every other triangle faces backwards
  if(islower){
    lightPointCosAngle *= -1;
  }
  ...
  if(point /*&& lightPointCosAngle > */){ //side is only affected by light source if it is facing it
    //console.log("lightcoor is: [" + lightcoor[0] + ", " + lightcoor[1] + ", " + lightcoor[2] + "] ")
    //will add 3d point lating when more time permits, for now 2D will function

    var dist = Math.sqrt( (thiscor.x - lightcoor[0])**2 + (thiscor.y - lightcoor[1])**2 + (thiscor.z - lightcoor[2])**2)
    redcol += Number(rgb[0] * lightcol[0])/(140*(1+dist));
    greencol += Number(rgb[1] * lightcol[1])/(140*(1+dist));
    bluecol += Number(rgb[2] * lightcol[2])/(140*(1+dist));
  ...
  if(specular && directional){
    ...
    var Nmag = Math.sqrt(normal.x**2 + normal.y**2 + normal.z**2);
    //var midDotProduct = midvector[0]*thiscor.x + midvector[1]*thiscor.y + midvector[2]*thiscor.z;
    var midDotProduct = midvector[0]*normal.x + midvector[1]*normal.y + midvector[2]*normal.z;
  ...
  if(specular && point){
    //finds halfway vector 
    camvector = [0,0,1];
    //direct vector = angle of light, between surface and light point
    //directvector = pointcol;
    //directvector = [1,1,1];
    pointloc
    var dv = [lightcoor[0] - thiscor.x, lightcoor[1] - thiscor.y, lightcoor[2] - thiscor.y]
    var dvmag =  Math.sqrt(dv[0]**2 + dv[1]**2 + dv[2]**2);
    directvector = [dv[0]/dvmag, dv[1]/dvmag, dv[2]/dvmag];
    midvector = [(camvector[0] + directvector[0])/2, (camvector[1] + directvector[1])/2, (camvector[2] + directvector[2])/2];
    ...
    var midDotProduct = midvector[0]*normal.x + midvector[1]*normal.y + midvector[2]*normal.z;
    var Nmag = Math.sqrt(normal.x**2 + normal.y**2 + normal.z**2);
    var distance = Math.sqrt( (thiscor.x * lightcoor[0])**2 + (thiscor.y * lightcoor[1])**2 + (thiscor.z * lightcoor[2])**2 )
    var midCosAngle = midDotProduct/(Nmag * midMag);
    var distance_factor = (1 - distance) ;
    var dist = Math.sqrt( (thiscor.x - lightcoor[0])**2 + (thiscor.y - lightcoor[1])**2 + (thiscor.z - lightcoor[2])**2)
    ...
    //console.log("midcosangle is: " + midCosAngle + ", middotproduct is: " + midDotProduct + ", Normalmag is: " + Nmag
    //+ ", midMag is: " + midMag + ", distance is: " + distance + ", distance factor is: " + distance_factor);
    if(midCosAngle > 0){
      //redcol += 255 * 6.5 * (midCosAngle **4) * distance_factor;
      //greencol += 255 * 6.5 * (midCosAngle **4)* distance_factor;
      //bluecol += 255 * 6.5 * (midCosAngle **4)* distance_factor;
      redcol += 255 * 16 * (midCosAngle **4)/(15*(1+dist));
      greencol += 255 * 16 * (midCosAngle **4)/(15*(1+dist));
      bluecol += 255 * 16 * (midCosAngle **4)/(15*(1+dist));
      //console.log("color increased by: " + (midCosAngle **4)/(150*(1-dist)))
    }
  }
...
function MakeBackground(contxt){
  var rgbr = Number(document.getElementById("bred").value);
  var rgbg = Number(document.getElementById("bgreen").value);
  var rgbb = Number(document.getElementById("bblue").value);

  var redstr = Math.round(rgbr).toString(16).toUpperCase();
        if(redstr.length == 1){
          redstr = "0" + redstr;
        }
        var greenstr = Math.round(rgbb).toString(16).toUpperCase();
        if(greenstr.length == 1){
          greenstr = "0" + greenstr;
        }
        var bluestr = Math.round(rgbg).toString(16).toUpperCase();
        if(bluestr.length == 1){
          bluestr = "0" + bluestr;
        }
        sidecolor = "#" + redstr + greenstr + bluestr;
  contxt.fillStyle = sidecolor;
  contxt.fillRect(0, 0, 500, 500);
}

function Animate(){
  
  
  //console.log("animate is being run, animx is: " + animx);
  if(allow_new_points == false){
    //console.log("animate run, animate is: " + animx);
    if(animx){
      //console.log("xcamvalue is: " + document.getElementById("xcam").value + " type is: " + typeof(document.getElementById("xcam").value ));
      document.getElementById("xcam").value = (Number(document.getElementById("xcam").value) + 2).toString();
      if(Number(document.getElementById("xcam").value) >= 360){
        document.getElementById("xcam").value = "0"
      }
    }
    if(animy){
      document.getElementById("ycam").value = (Number(document.getElementById("ycam").value) + 2).toString();
      if(Number(document.getElementById("ycam").value) >= 360){
        document.getElementById("ycam").value = "0"
      }
    }
    ChangeCam();
  }
...
var interval = setInterval(Animate, 35);